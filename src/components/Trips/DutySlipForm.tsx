...
export function DutySlipForm({
  ...
  useEffect(() => {
    ...
    const shouldSync = trip?.calculated_amount == null ? Number(trip?.trip_amount ?? 0) === 0 : Number(trip?.trip_amount ?? 0) === Number(trip.calculated_amount);
  ...
  const selectedDriver = drivers.find((driver) => driver.id === formState.driver_id) ?? null;
  const driverDefaultVehicle = vehicles.find((vehicle) => vehicle.id === selectedDriver?.default_vehicle_id) ?? null;
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    await onSave(
      {
        trip_number: formState.trip_number,
    ...
    <section className="space-y-5 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h3 className="mt-2 text-2xl font-semibold text-slate-900">{trip ? `Edit ${trip.trip_number}` : 'Create GT trip'}</h3>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="grid gap-4 lg:grid-cols-4">
          <label className="text-sm font-semibold text-slate-800">Trip Number<input value={formState.trip_number} onChange={(event) => setFormState((current) => ({ ...current, trip_number: event.target.value }))} className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 font-normal" required /></label>
          ...
          <label className="text-sm font-semibold text-slate-800">Customer<select value={formState.customer_id} onChange={(event) => setFormState((current) => ({ ...current, customer_id: event.target.value }))} className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 font-normal" required><option value="">Select customer</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></label>
          <label className="text-sm font-semibold text-slate-800">Vehicle<select value={formState.vehicle_id} onChange={(event) => { const nextVehicleId = event.target.value; const nextVehicle = vehicles.find((vehicle) => vehicle.id === nextVehicleId) ?? null; setFormState((current) => ({ ...current, vehicle_id: nextVehicleId, vehicle_category_id: nextVehicle?.vehicle_category?.id ?? current.vehicle_category_id })); }} className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 font-normal" required><option value="">Select vehicle</option>{vehicles.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.vehicle_number}</option>)}</select></label>
          ...
          <label className="text-sm font-semibold text-slate-800">GT Category<select value={formState.vehicle_category_id} onChange={(event) => setFormState((current) => ({ ...current, vehicle_category_id: event.target.value }))} className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 font-normal"><option value="">Select category</option>{vehicleCategories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>
        ...
        {driverDefaultVehicle ? <p className="text-sm text-slate-500">Driver default vehicle: {driverDefaultVehicle.vehicle_number}</p> : null}
        ...
        <div className="flex flex-wrap gap-3">
          <button type="submit" disabled={saving} className="rounded-2xl bg-slate-900 px-5 py-3 text-sm font-medium text-white disabled:opacity-60">{saving ? 'Saving...' : trip ? 'Update Trip' : 'Create Trip'}</button>
      ...
      <div className="space-y-4 rounded-3xl border border-slate-200 bg-slate-50 p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="text-right text-sm text-slate-600"><div>Total KM: <span className="font-semibold text-slate-900">{formatMetricKm(totalMetricKm)}</span></div><div>Total Hours: <span className="font-semibold text-slate-900">{formatMetricHours(totalMetricHours)}</span></div></div>
        </div>
        ...
        {trip?.id ? (
          <form onSubmit={handleMetricSubmit} className="grid gap-4 lg:grid-cols-4">
            ...
            <div className="flex gap-3 self-end"><button type="submit" disabled={metricSaving} className="rounded-2xl bg-slate-900 px-5 py-3 text-sm font-medium text-white disabled:opacity-60">{metricSaving ? 'Saving...' : editingMetricId ? 'Update Row' : 'Add Row'}</button>{editingMetricId ? <button type="button" onClick={() => { setEditingMetricId(null); setMetricState(buildMetricFormState(trip)); }} className="rounded-2xl border border-slate-300 px-5 py-3 text-sm font-medium text-slate-700">Cancel</button> : null}</div>
          </form>
        ) : <p className="text-sm text-slate-500">Create the trip first, then add travel metrics and run calculation.</p>}

        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            <thead className="bg-slate-50 text-left text-slate-600"><tr><th className="px-4 py-3">Seq</th><th className="px-4 py-3">Start</th><th className="px-4 py-3">End</th><th className="px-4 py-3">Segment KM</th><th className="px-4 py-3">Segment Hours</th><th className="px-4 py-3">Action</th></tr></thead>
            <tbody className="divide-y divide-slate-100 bg-white">{(trip?.metrics ?? []).map((metric) => <tr key={metric.id}><td className="px-4 py-3 font-medium text-slate-900">{metric.seq}</td><td className="px-4 py-3 text-slate-600">{metric.start_date} {metric.start_time}<div className="text-xs text-slate-500">KM {metric.start_km}</div></td><td className="px-4 py-3 text-slate-600">{metric.end_date && metric.end_time ? `${metric.end_date} ${metric.end_time}` : 'Open'}<div className="text-xs text-slate-500">KM {metric.end_km ?? '-'}</div></td><td className="px-4 py-3 text-slate-600">{formatMetricKm(metric.segment_km)}</td><td className="px-4 py-3 text-slate-600">{formatMetricHours(metric.segment_hours)}</td><td className="px-4 py-3"><button type="button" onClick={() => { setEditingMetricId(metric.id); setMetricState(buildMetricFormState(trip, metric)); }} className="rounded-xl border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-700">Edit</button><button type="button" onClick={() => void handleDeleteMetric(metric)} className="ml-2 rounded-xl border border-rose-200 px-3 py-1.5 text-xs font-medium text-rose-700">Delete</button></td></tr>)}{!(trip?.metrics.length) ? <tr><td colSpan={6} className="px-4 py-6 text-center text-slate-500">No travel metrics added yet.</td></tr> : null}</tbody>
          </table>
        </div>
