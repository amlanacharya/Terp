...
describe('PostgresService', () => {
    ...
    let mockExec;
    beforeEach(() => {
        service = new postgres_service_1.PostgresService();
        mockExec = child_process_1.exec;
        jest.clearAllMocks();
    });
    describe('getStatus', () => {
    ...
    describe('start', () => {
        ...
        it('should handle already started service gracefully', async () => {
            mockExec.mockImplementation((cmd, callback) => {
                if (typeof cmd === 'string' && cmd.includes('net start')) {
                    const error = new Error('The service has already been started');
    ...
    describe('stop', () => {
        it('should stop PostgreSQL service successfully', async () => {
            ...
            expect(mockExec).toHaveBeenCalledWith(expect.stringContaining('net stop'), expect.any(Function));
        });
