...
export function DriverSettlements() {
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      ...
      setError('');
      await api.delete(`/settlements/drivers/${deleteTarget.id}`);
  ...
  return (
    <section className="space-y-4">
      ...
      <Modal
        ...
        title={editingId ? 'Edit Salary Slip' : 'Add Salary Slip'}
        size="xl"
      ...
      >
        <form onSubmit={handleSubmit} className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
          ...
          <div className="xl:col-span-3 flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={saving} className="rounded-2xl bg-slate-900 px-5 py-3 text-white disabled:opacity-60">{saving ? 'Saving...' : editingId ? 'Update Salary Slip' : 'Create Salary Slip'}</button>
          </div>
        </form>
      </Modal>
