      ...
      {canManage ? (
        <form onSubmit={handleSubmit} className="grid gap-4 rounded-3xl border border-slate-200 bg-slate-50 p-5 lg:grid-cols-4">
          <label className="text-sm font-semibold text-slate-800">
            Vehicle Number
            <input value={formState.vehicle_number} onChange={(event) => setFormState((current) => ({ ...current, vehicle_number: event.target.value }))} placeholder="Vehicle number, e.g. MH01AB1234" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 font-normal" required />
          </label>
          <label className="text-sm font-semibold text-slate-800">
            Vehicle Type
            <select value={formState.vehicle_type} onChange={(event) => setFormState((current) => ({ ...current, vehicle_type: event.target.value }))} className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 font-normal">
          ...
          <label className="text-sm font-semibold text-slate-800">
            Make
            <input value={formState.make} onChange={(event) => setFormState((current) => ({ ...current, make: event.target.value }))} placeholder="Vehicle make, e.g. Tata" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 font-normal" />
          </label>
          <label className="text-sm font-semibold text-slate-800">
            Model
            <input value={formState.model} onChange={(event) => setFormState((current) => ({ ...current, model: event.target.value }))} placeholder="Vehicle model, e.g. Winger" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 font-normal" />
          </label>
          <label className="text-sm font-semibold text-slate-800">
            Year
            <input value={formState.year} onChange={(event) => setFormState((current) => ({ ...current, year: event.target.value }))} placeholder="Year, e.g. 2022" type="number" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 font-normal" />
          </label>
          <label className="text-sm font-semibold text-slate-800">
            Seating Capacity
            <input value={formState.seating_capacity} onChange={(event) => setFormState((current) => ({ ...current, seating_capacity: event.target.value }))} placeholder="Seats, e.g. 17" type="number" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 font-normal" />
          </label>
          ...
          <div className="flex gap-3">
            <label className="flex-1 text-sm font-semibold text-slate-800">
              Vendor Owner
              <select value={formState.owner_id} onChange={(event) => setFormState((current) => ({ ...current, owner_id: event.target.value }))} disabled={formState.is_owned} className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 font-normal disabled:bg-slate-100">
