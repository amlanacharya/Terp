...
export function TaxComponentList() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<TaxComponent | null>(null);
  ...
  return (
    <section className="space-y-4">
      ...
      <Modal
        ...
        title={editingId ? 'Edit Tax Component' : 'Add Tax Component'}
        size="md"
      ...
      >
        <form onSubmit={handleSubmit} className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="lg:col-span-2 flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={saving} className="rounded-2xl bg-slate-900 px-5 py-3 text-white disabled:opacity-60">{saving ? 'Saving...' : editingId ? 'Update Tax Component' : 'Create Tax Component'}</button>
          </div>
        </form>
      </Modal>
      ...
      <ConfirmModal
        ...
        message={deleteTarget ? `Delete tax component ${deleteTarget.component_code}?` : ''}
        confirmLabel="Delete"
